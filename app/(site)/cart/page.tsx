...
export default function CartPage() {
  ...
  if (items.length === 0) {
    return (
      <div className="mx-auto max-w-2xl px-6 py-24 text-center">
        ...
        <p className="mt-3 text-stone">Add a product to get started.</p>
        <Link
          href="/"
  ...
  return (
    <div className="mx-auto max-w-3xl px-6 py-16">
        ...
        {items.map((item) => (
          <div key={item.productId} className="flex items-center gap-4 py-5">
            <div className="relative h-20 w-20 flex-shrink-0 border border-line bg-ink">
              {item.imageUrl && (
                <Image src={item.imageUrl} alt={item.name} fill sizes="80px" className="object-contain p-2" />
              )}
            </div>
      ...
      <p className="mt-1 text-sm text-stone">Shipping costs are calculated at checkout.</p>
